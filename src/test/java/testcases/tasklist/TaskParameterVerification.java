...
public class TaskParameterVerification extends BaseTest {
	@Test
	public void taskPage() throws IOException, InterruptedException {
		...
		test.log(Status.INFO, "Verifying whether the created task matches the task title or not");
		tp.compareTaskTitle(randomTaskTitle, newtasktitle);

		boolean dueDateMatch = TaskPageResources.verifyDueDate(tp);

		if (dueDateMatch) {
			test.log(Status.PASS, "Due date before and after task creation matches.");
		} else {
			test.log(Status.FAIL, "Due date before and after task creation do not match.");
		}

		Thread.sleep(3000);
		test.log(Status.INFO, "Verifying whether the task is Cancelling or not");

...
import testcomponents.WaitUtility;

public class ProjectListingPage extends BaseTest {

	@Test
	public void measureProjectListingResponseTime() throws IOException, InterruptedException {
		...
		for (int i = 0; i < iterations; i++) {
			double responseTimeInSeconds = PageResponseTimeUtil.measureResponseTimeInSeconds(() -> {
				...
				try {
					...
					wu.waitForElementVisibility(loginPage.email);
				} catch (InterruptedException e) {
					e.printStackTrace();
				}
			});
